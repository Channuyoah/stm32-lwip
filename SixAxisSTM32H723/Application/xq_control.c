...
// 处理“32路DI输入”（保持寄存器地址 1642）
static void on_DI32_Input(uint16_t addr) {
    uint16_t val = usRegHoldBuf[addr - MB_HOLD_START_ADDR];
    printf("------DI32 changed, addr=%d, value=0x%X\r\n", addr, val);
    // 实际刷新 IO 的代码放在这里
    usRegHoldBuf[addr - MB_HOLD_START_ADDR]=xqIO_Input;
}
...
void xq_register_all_handlers(void) {
    ...
    xq_reg_register_handler(1642, on_DI32_Input);
